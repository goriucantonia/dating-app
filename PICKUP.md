...
**Last updated:** 2026-09-02 · **Updated because:** **One owner decision landed on the date pipeline: the date scenario is now drawn at random ONCE per analysis and run identically against every candidate (D-021).** Before this, three candidates were ranked against each other on three DIFFERENT evenings and the results masthead printed "— the same for every candidate" underneath them. Now one archetype is drawn in code from a 16-entry catalogue, generated once with no personal detail in the call, and copied onto every date, with the last 3 a user has had excluded from their next draw. **Server 142 pytest GREEN · UX 51 widget tests GREEN · `flutter analyze` clean · migration `0011` applied. OWED: the live witness — nothing in this change has been run against real providers yet (see "What Step 11 owes after 2026-09-02").** Earlier today: D-020, D-019, D-018, D-017, Step 17 (candidate rejection, probe GREEN 15/15), D-016, the Modernist restyle (O-18), the ten-person demo pool (O-22).
...
- **`ended_by` and `excluded_from_score` come from the server and are never re-derived in the client.** A client with its own idea of "mutual" or of the 10-turn rule is a client that can disagree with the score it is showing. The server computes both with the same functions the pipeline used. *(`fixture` was added under the same principle on 2026-09-02.)*
...
## What Step 11 owes after 2026-09-02

**This owner decision was built today and has NOT been witnessed live.** Everything below is green in unit tests and in the database; not one model call has been made under the new design. Do not describe it as done in front of anyone until the run in "The witness this owes" has happened.
...
- **The wire.** `GET /analyses/{id}/dates` gains a top-level `fixture` object (`setting_name`, `archetype`, `dates_per_candidate`; null on pre-2026-09-02 analyses) and each date carries `archetype` instead of `anchored_in_interest`.
- **The screen.** The masthead prints the fixture and says "every candidate went here, so these scores compare" — and on an older analysis says "a different one per candidate", which is what was actually true of it.

**Backward compatibility, deliberate and load-bearing:** no backfill was attempted. Analyses that ran before today genuinely had no shared fixture, and writing one now would be inventing history. They render as absence.

**The witness this owes (O-23).** One analysis end-to-end against real providers, ~20 calls, checking three things no unit test can:
...
`probes/probe_judge.py` already checks 1 against whatever is stored, and costs one judging call.
