...
- **S11-B5** Caps, exactly: **1 date per candidate, max 3 per analysis, 16 agent TURNS per date** *(all REVISED 2026-09-01, owner decision; were 2, 6, and a 30-message cap)* — and the turn cap **does NOT count environment rows**, because one turn is one model call and an event costs none (`§18`, and note that §18's own example was this rule the other way round until the same date).
