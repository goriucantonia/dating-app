...
| D-031 | A new provider silently un-fixed D-008 for every task routed through it | server | closed — found by the first live call, fixed and re-witnessed the same hour |
...
- **Status:** closed (2026-09-02) in code, tests and documents; **the live witness is owed** — no end-to-end run against real providers has been made since the change (see `PICKUP.md`). Server suite 145 green, UX suite 54 green, `flutter analyze` clean, migrations `0011` and `0012` applied.
...
## D-031 — a new provider silently un-fixed D-008 for every task routed through it

- **Date:** 2026-09-04
- **Repo:** `server`
- **Surfaced in:** the FIRST live call ever made through the new `LiteLLMProvider`, added the same hour.
- **Mechanism:** `openrouter.py` classifies an OpenRouter body containing `"Provider returned error"` as **`TransientAIError`**, on purpose and with a long comment: one OpenRouter model id is served by several upstreams chosen per request, so that body means "the upstream we drew failed", and a retry is a fresh routing draw (D-008). LiteLLM reaches the same aggregator but flattens that upstream fault onto its own `BadRequestError`. My error mapper read `BadRequestError` and returned a plain `AIError` — **fatal, never retried**. So the single most common OpenRouter fault in this project became a hard failure for anything routed through the new provider, while the identical fault on the identical upstream stayed correctly retryable through the old one. Nothing in the unit tests could see it: they assert the mapping I wrote against a fake whose exception classes I also wrote, so they agreed with me.
- **Discovery method:** A live run, on the first call. `probe`-style unit tests were green (42 of them) and stayed green while this was broken. The failing call returned the exact `{"message":"Provider returned error", "provider_name":"AtlasCloud"}` body that D-008 is about, which is what made the misclassification obvious rather than merely wrong.
- **Lesson:** Two, and the first is the one that generalises.
  1. **A new provider does not inherit the defect knowledge the old ones encode — it has to re-earn it.** Every hard-won classification in `openrouter.py` and `google.py` is a fact about an UPSTREAM, not about the client library that reached it, so a second client library reaching the same upstream meets the same faults with none of the fixes. The mechanical version of this check: before adding a provider, read the existing providers' error mapping for comments that name a defect, and ask which of them are properties of the upstream rather than of the SDK. Here that was one of two, and it was load-bearing.
  2. **A unit test whose fixtures you also wrote tests your model of the world, not the world.** The fake litellm asserted the exception hierarchy correctly and the mapping passed against it; what it could not assert was which real exception class a real aggregator fault would arrive as. One live call answered that, and the real hierarchy was verified against the installed package afterwards rather than assumed.
- **Status:** closed (2026-09-04). `_is_upstream_fault()` matches the aggregator's "the upstream failed" vocabulary and returns `TransientAIError`, checked BEFORE the `response_format` case exactly as `openrouter.py` orders it, so one bad routing draw cannot blacklist a model's native schema support for the process. Re-witnessed live: the same call now logs three `ai_call_retry` lines with `schedule: transient` and an honest typed give-up. Three tests pin it, including that a genuine bad request stays fatal — the classification has to stay narrow or a real request error is retried three times to fail identically.
- **Cross-refs:** D-008 (the original, and the reason this was recognisable on sight); `server/app/ai/litellm_provider.py`, `server/app/ai/openrouter.py`, `server/tests/test_litellm_provider.py`; `ai_interaction.md` §3.
