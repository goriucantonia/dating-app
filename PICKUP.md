...
**Demo accounts (password `demo-password`, from `seeds/demo_profiles.yaml`, all opted IN by design — they are the pool):** `maya.demo@dating-demo.dev`, `theo.demo@dating-demo.dev`, `jonah.demo@dating-demo.dev`. Reconciliation recreates any of them that goes missing, through the real pipeline.
