...
**Last updated:** 2026-09-01 · **Updated because:** **Step 14 (chat) is BUILT, unit- and widget-tested on mocks, and its server side witnessed over HTTP** — selection pins the matched snapshot and refuses a second choice as state; one real reply came back through `agent_response.v1` with NO `state` on the wire and the state stored; the forced give-up returns `502 reply_failed` with the user's message NOT stored (after D-014, found by that very forced run); ending a session leaves the history readable and refuses new messages as state. Compaction is unit-tested at its boundary and NOT witnessed live (O-19 — it costs 30+ model calls). Step 13 before it: built and tested on mocks, browser pass owed (O-16). Both gates CLOSED; O-8, O-13, O-14 unchanged.
...
**Next: Step 15 (data hygiene: deletion, demo seeding, the full reconciliation pass, the dead-data scan).** Steps 1–8, 10 and 12 are done and witnessed; Step 9 is code-complete with its witness OWED (O-8); Step 11 is built with its central claim witnessed; **Steps 13 and 14 are built, tested on mocked data, and wire-verified on real data — their signed-in browser passes are OWED (O-16)**. Both gates are closed.
...
| Unit tests | **Server 112 pass** (21 new in `tests/test_chat_rules.py` and `tests/test_simulate_retry.py`) · **UX 25 pass** (`test/step13_results_test.dart`, `test/step14_chat_test.dart` — all on mocked repositories, no server, no model calls) | in-container `pytest`; `flutter test` |
...
**Step 15 — data hygiene — is next**: `DELETE /me` over the whole cascade graph (now including `chat_sessions`/`chat_messages`), the per-table counts logged BEFORE the cascade and returned as the receipt, the survivor-side tombstones (dangling analyses, chat lists), `seeds/demo_profiles.yaml` seeded through the REAL pipeline (2 calls per demo user), the full four-step reconciliation pass, `scripts/scan_dead_data.py`, and the two probes. **Mind the chat cascade** (`chat.md`): a deleted candidate takes their sessions with them from the selecting user's list — that is the tombstone case for the chat list.
