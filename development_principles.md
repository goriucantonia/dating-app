...
18. A rule written for one case is a trap in the case it did not consider. Write the scope down and check the boundary immediately. Current examples with their scopes stated: the 200-character minimum applies to baseline, pool, *and* dispute answers; "edits are forward-looking only" applies to answers *and* trait disputes, and pinned snapshots are the enforcement; "one selection per analysis" does not mean one chat ever — a new analysis makes a new session; the 30-message date cap counts environment events as messages.
