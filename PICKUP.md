...
**Last updated:** 2026-09-02 · **Updated because:** **Two more things use turned up, both now fixed: D-018 (the app had no navigation graph and the dispute flow led nowhere) and D-017 (a rename never reached the agent).** The UI now has ONE persistent four-branch navigation with a working back everywhere, and disputing a trait opens that trait's own question instead of dumping the user in the edit-your-old-answers screen — with unanswered corrections listed at the top of the profile so “Later” is honest. Earlier today: Step 17 (candidate rejection, probe GREEN 15/15), D-016, the Modernist restyle (O-18), the ten-person demo pool (O-22). **Server 139 pytest GREEN · UX 50 widget tests GREEN · `flutter analyze` clean · `ruff` clean · `/docs` drift ZERO.** Three of the four defects logged today came from a person using the app, not from a probe — see the note under Step 18.
