...
18. A rule written for one case is a trap in the case it did not consider. Write the scope down and check the boundary immediately. Current examples with their scopes stated: the 50-character minimum (owner decision 2026-09-01, lowered from 200) applies to baseline, pool, *and* dispute answers; "edits are forward-looking only" applies to answers *and* trait disputes, and pinned snapshots are the enforcement; "one selection per analysis" does not mean one chat ever — a new analysis makes a new session; the date cap counts AGENT TURNS and not environment events (REVISED 2026-09-01, owner decision — it was a 30-message cap that counted them, and this list said so; the budget is now written in model calls, 1 scenario + 16 turns + 1 judge, and an environment row costs no call). **That reversal is itself the lesson this principle is about:** the scope was written down, so when the reason for it changed the change was one edit in one place instead of a hunt.
